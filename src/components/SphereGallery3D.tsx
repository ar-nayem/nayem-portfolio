 ...
 *
 * RAW WebGL, no `three` dependency: everything a library would provide here
 * — perspective projection, the YXZ camera rig, quaternion billboarding, the
 * pointer ray, sphere tessellation, texture upload — is ~200 lines of
 * arithmetic, so it lives in this file.
 *
 * Architecture: the GL context is built ONCE in `useEffect([])`, every live
 * prop is read from a ref inside a raw rAF loop, and cleanup never calls
 * `loseContext()` (getContext returns the same context per canvas, so a
 * StrictMode remount would reuse a force-lost one).
 *
 * Interaction:
 ...
 *   • click       — raycast, open that node's link in a new tab.
 */
...
const dummyImage = (i: number, w = 1200, h = 800) => {
  ...
  return (
     ...
     * renders empty. Encoding them keeps ONE string usable as an <img> src,
     * a CSS url() and a GL texture alike. */
    .replace(/ /g, "%20")
    .replace(/'/g, "%27")
    .replace(/\(/g, "%28")
    .replace(/\)/g, "%29");
...
const RADIUS = 12; // shell radius, world units
const DAMPING = 0.6; // spring settle, 0-1
const FOV = 45; // ° vertical
...
const SPHERE_W = 32; // core tessellation
const SPHERE_H = 24;
const NEAR = 0.1;
const FAR = 2000;
...
/* -------------------------------------------------------------------- colour
 * Robust parse: #rgb, #rgba, #rrggbb, #rrggbbaa, rgb(), rgba(). Values stay
 * in sRGB and go straight to the framebuffer.
 */
...
/**
 * Shortest rotation taking +Z onto `dir` (a unit vector), specialised to
 * vFrom = (0,0,1) — the quad's own normal.
 */
...
/** quaternion slerp, including the shortest-path flip */
...
/* ------------------------------------------------------------ core geometry
 * A UV sphere generated inline: SphereGeometry(1, 32, 24) equivalent. The
 * halo reuses the same positions with a (deliberately non-deduped) edge index.
 */
function buildSphere(widthSeg: number, heightSeg: number) {
  ...
  // 2x brighter lattice — measured at 1.85-1.90x brighter than a deduped
  // set, which is the intended look of the halo.
  const edges: number[] = [];
...
export default function SphereGallery3D(props: SphereGallery3DProps) {
   ...
   * setup effect's deps would rebuild the GL context on every prop change.
   */
  const radius = RADIUS;
   ...
   * ABOVE — the axis the sphere actually turns about — is anticlockwise. */
  ...
  // Refs are for the rAF loop, not render output, so they're written in a
  // layout effect (after commit, before paint) rather than during render —
  // mutating a ref mid-render is undefined under concurrent rendering.
  React.useLayoutEffect(() => {
    live.current = {
      ...
      // 50 is the rate the sphere was tuned at; Direction is the sign
    ...
    // one entry per BRANCH, cycled like the images — the hit test returns a
    // node index, so this table has to be node-indexed too
  ...
  React.useEffect(() => {
    ...
    items.forEach((item, i) => {
      ...
      if (!url) return;
      const img = new Image();
      // a tainted canvas cannot be used as a texture source at all
    ...
    // graveyardRef.current is a queue mutated in place (push/pop), never
    // reassigned, so capturing the array reference now is the same array
    // the cleanup below will push into
    ...
    return () => {
      ...
      loaded.forEach((l) => {
        if (l.texture) grave.push(l.texture);
        l.texture = null;
      });
    };
  ...
  React.useEffect(() => {
    ...
    // captured once: this effect runs only on mount ([] deps below), and the
    // array is mutated in place (push/pop) and never reassigned
    const grave = graveyardRef.current;
    ...
    const upload = (img: HTMLImageElement): WebGLTexture | null => {
      ...
      // SRGB8_ALPHA8 on a WebGL2 context: the sampler decodes sRGB -> linear
      // in hardware, and because this shader writes its result straight
      // out with no encode, the photos land on screen linearised — darker
      // and more contrasty than the source file. That is the intended look
      // (measured against a plain gl.RGBA upload: mean |delta| 0.18/255 vs
      // 6.10/255). To show photos at true sRGB instead, pass gl.RGBA as the
      // internal format below — nothing else changes.
      gl.texImage2D(
        gl.TEXTURE_2D,
        0,
        isGL2 ? (gl as unknown as WebGL2RenderingContext).SRGB8_ALPHA8 : gl.RGBA,
        gl.RGBA,
        gl.UNSIGNED_BYTE,
        img
      );
      ...
      if (mip) {
        ...
        if (aniso) {
          const max = gl.getParameter(
            (aniso as { MAX_TEXTURE_MAX_ANISOTROPY_EXT: number })
              .MAX_TEXTURE_MAX_ANISOTROPY_EXT
          );
          gl.texParameterf(
            gl.TEXTURE_2D,
            (aniso as { TEXTURE_MAX_ANISOTROPY_EXT: number })
    ...
    const frame = () => {
      ...
      // repeated image stays ONE decoded texture.
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (hovering) {
          ...
          const wx = tmpA[0],
            wy = tmpA[1],
            wz = tmpA[2];
          // closest point on the ray (t clamped at the origin, never
          // behind the camera)
          const t = Math.max(
            0,
            (wx - rayO[0]) * rayD[0] +
        ...
        // frozen at 60% — the decay rate, not a per-frame multiply, so
        // it stays correct at any frame rate.
      ...
      const drawQuads = (from: number, to: number) => {
        for (let o = from; o < to; o++) {
          ...
          gl.bindTexture(
            gl.TEXTURE_2D,
            hasTex ? (m as Loaded).texture : white
          );
          ...
          // --- depth shading -------------------------------------
          const depth = R[6] * nd.px + R[7] * nd.py + R[8] * nd.pz; // world z
          ...
          gl.uniform1f(
            qLoc.uAA,
            (worldPerPixel * dist) / Math.max(1e-4, sy)
          );
    ...
    return () => {
      ...
      // mediaRef.current is read live (not captured above): the media effect
      // reassigns it wholesale on every mediaKey change, so only a read at
      // cleanup time sees the current array
      mediaRef.current.forEach((m) => {
        if (m.texture) gl.deleteTexture(m.texture);
        m.texture = null;
      });
      // graveyardRef.current is mutated in place and never reassigned, so
      // this identity is the same array the whole component lifetime
      while (grave.length) {
        gl.deleteTexture(grave.pop() as WebGLTexture);
      }
